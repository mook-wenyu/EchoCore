...
- 单元测试 **381 个全绿**（23 文件，三次连跑稳定）；类型检查与构建通过
...
- **远程嵌入 dimensions + 状态可见化**（本次提交）：用户实测报障「嵌入状态 ready 但配置写了没生效」——实测钉死根因：`remoteEmbedFetch` 请求体不带 `dimensions` → 端点回默认维度（qwen3.7-text-embedding=1024）≠ 配置 2048 → 维度强校验拦截 → bootstrap 静默回退本地 384（无任何用户可见提示）。修复：①请求体显式携带 `dimensions: config.dimension`（实测带 2048 端点正确返回 2048——OpenAI text-embedding-3 / 百炼 qwen3 兼容层标准参数）；②`EmbeddingService.lastInitError` 记录远程验证失败原因（init 重置，不静默）；③面板状态行显示 `嵌入状态：ready（后端：remote|local）` + 远程未生效原因警告行 + 保存后自动刷新（RuntimeHealth 加可选 embeddingBackend/embeddingInitError，host-rpc status 透传）；④保存文案改为指向顶部状态行（不再写死"已热切换"）。381 测试全绿（+6：dimensions 请求体断言、lastInitError 记录/重置、RPC 透出×2、面板渲染×2）
- **接口契约变更**（累计）：配置项 19→4；`InjectorConfig/ExtractorConfig/SnapshotConfig/MaintenanceConfig` 接口删除；`MemoryStableSnapshot.EMPTY_IDS` 删除；CONFIG_DICT 改读 `Config.dict`（transform 包装移除后）；EmbeddingIndexDeps.service 加 `dimension`；`createMemoryRpcHandler(store, rpc)`/`registerMemoryRpc(ctx, store, config)` → `registerMemoryRpc(ctx, store, rpc)`（rpc 由 `{config: ResolvedConfig, fiber}` 改 `{config(), settings, applyChange}`）；`MountOverrides` 加 `seam`；`MemoryInjectorDeps`/`MemoryToolsDeps` 的 `embedding`+`embedIndex` 合并为 `embedding: EmbeddingHolder`（调用时读，热换即生效）；`EmbeddingServiceDeps.fetchRemoteEmbeddings` 加可选 `opts?: EmbeddingFetchOptions`；`SettingsSeam` 加 `setApplier`；`remoteEmbedFetch` 加 `opts` 参数（timeoutMs/retries）且请求体带 `dimensions`；`EmbeddingService` 加 `lastInitError`；`RuntimeHealth` 加可选 `embeddingBackend/embeddingInitError`；`MemoryPanelApi` 加 getConfig/setConfig；`MemoryPanelConfigView` 仅 5 字段
...
11. **400K 自动压缩模型策略漂移（用户环境配置，非插件代码）**：compaction-basic 的 `modelPolicies` 是 provider+model 精确匹配（已读源码 dsh-compaction-basic lib/index.js:84 `find(p => p.provider===target.provider && p.model===target.model)`）；patch 只配 opencode-go，而默认模型已是 teamorouter/deepseek-v4-flash（settings.yaml agent-default-model）→ 0.4 未命中 → 回落默认 0.8 → 触发点 800K 而非 400K（用户观察"长会话不再 400K 自动压缩"吻合）。修复=patch 补 `teamorouter/deepseek-v4-flash` 策略（等用户拍板）。

## 四、下次最该做的事

1. **重启 3080 实例**：当前进程仍运行旧代码（0:36 启动，早于 A1/B1 与本次 dimensions 修复的部署）——重启后新代码生效；重启后到设置页「记忆」保存一次配置，确认：`~/.dsh/settings.yaml` 出现 `memory:` 段、cordis.yml 保持 []、**保存不崩溃且立即生效**，且顶部「嵌入状态」行显示真实后端（`ready（后端：remote）`——dimensions 修复后远程 2048 维真正生效；若仍显示 local 顶班则警告行会给出失败原因）。
2. **400K 自动压缩策略补位（用户环境）**：默认模型已切 teamorouter，patch 的 compaction-basic modelPolicies 需补 `{provider: teamorouter, model: deepseek-v4-flash, thresholdRatio: 0.4}`（或等价）恢复 400K 触发（含 pro/free 各模型建议一并补）。
