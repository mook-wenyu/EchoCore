...
| 反思自进化 | 维护周期自动 + 手动工具/RPC 触发 LLM 审视已有条目间**语义近似重复**与**跨条目矛盾**：只做可逆「归档一侧」动作（归档较旧、保留较新，审计 `by:system` + 依据 detail、可回滚）；**不做**内容改写 / 单次 importance 重打分 / 无来源合成 insight（依据 A′ 负面证据：Manufactured Confidence / Choice-Supportive Bias / Useful Memories Become Faulty）；跨轮累计（runs/合并/归档/跳过）经 `memory_status`/RPC status 透出（轻量质量钩子，观测"反思是否在收敛"）；**焦点策略（C29，2026-08-18 实证修复）**：按带内 token 重合度优先覆盖（防重要度 top-20 漏掉 imp6-7 真重复；无 peer 孤条目不占焦点、同对两端双向进集）；LLM 畸形输出（无 `decisions` 字段）显式 warn（与诚实 0 裁决可区分） |
