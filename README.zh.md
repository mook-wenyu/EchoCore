...
`@echocore/dsh-memory` 是 DSH（DeepSeek Harness，Cordis 生态）的记忆插件——每个会话自动从对话中提取记忆（事实/偏好/决策/待办/洞察），按相关性注入后续上下文，并提供模型工具（`memory_recall` / `memory_search` / `memory_note` / `memory_forget` / `memory_audit` / `memory_status`）与浏览器记忆面板。定位为**轻量 Cordis 插件**：无外部服务、SQLite 持久化（WAL 追加写，`$DSH_HOME/storages/memory.sqlite`）、关键词评分即用、语义嵌入可选。
